...
public class PadLayout extends ViewGroup {
    ...
    public PadLayout(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        TypedArray a =
                context.getTheme()
                        .obtainStyledAttributes(attrs, R.styleable.BoxGridLayout, 0, defStyleAttr);
        ...
        try {
            strokeWidth =
                    a.getDimensionPixelSize(
                            R.styleable.BoxGridLayout_separatorWidth, DEFAULT_STROKE_WIDTH);
        ...
        for (int i = 0; i < getChildCount(); i++) {
            ...
            if (!(childView instanceof DialPadKey) || ((DialPadKey) childView).isEqualSpacing()) {
                if (rowHeight > columnWidth) {
                    lp.topMargin =
                            lp.bottomMargin =
                                    (rowHeight - columnWidth + lp.leftMargin + lp.rightMargin) / 2;
                } else if (columnWidth > rowHeight) {
                    lp.leftMargin =
                            lp.rightMargin =
                                    (columnWidth - rowHeight + lp.topMargin + lp.bottomMargin) / 2;
                }
            }
    ...
    protected void dispatchDraw(Canvas canvas) {
        ...
        for (int i = paddingLeft;
                i <= width - paddingRight;
                i += (width - paddingLeft - paddingRight) / mColumnCount) {
            canvas.drawLine(i, paddingTop, i, height - paddingBottom, mPaint);
        }
        for (int i = paddingTop;
                i <= height - paddingBottom;
                i += (height - paddingTop - paddingBottom) / mRowCount) {
