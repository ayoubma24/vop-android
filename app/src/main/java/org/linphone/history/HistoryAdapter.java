...
public class HistoryAdapter extends SelectableAdapter<HistoryViewHolder> {
    ...
    public void onBindViewHolder(@NonNull final HistoryViewHolder holder, final int position) {
        ...
        holder.select.jumpDrawablesToCurrentState();
        if (position > 0) {
        ...
        holder.date.setText(
                secondsToDisplayableString(
                        String.valueOf(log.getStartDate()), holder.date.getContext()));
        holder.country.setText(
                getCountry(
                        LinphoneUtils.getAddressDisplayName(sipUri), holder.country.getContext()));
    ...
    private String secondsToDisplayableString(String callDate, Context mContext) {
        long longDate = Long.parseLong(callDate);
        return LinphoneUtils.timestampToHumanDate(
                mContext, longDate, mContext.getString(R.string.history_detail_date_format));
    }
    ...
    private String getCountry(String num, Context context) {
        PhoneNumberUtil phoneUtil = PhoneNumberUtil.createInstance(context);
        try {
            Phonenumber.PhoneNumber numberProto = phoneUtil.parse(num, null);
            return (phoneUtil.getRegionCodeForCountryCode(numberProto.getCountryCode()));
