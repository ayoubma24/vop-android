...
public abstract class AssistantActivity extends LinphoneGenericActivity
    ...
    void createProxyConfigAndLeaveAssistant(final boolean isGenericAccount) {
        final ProgressDialog progressdialog = new ProgressDialog(this);
        progressdialog.setMessage("Logging in....");
        progressdialog.show();

        Core core = LinphoneManager.getCore();
        final boolean useLinphoneDefaultValues =
        ...
        final ProxyConfig proxyConfig = getAccountCreator().createProxyConfig();
        ...
        mListener =
                new CoreListenerStub() {
                    ...
                    public void onRegistrationStateChanged(
                            ...
                            String smessage) {
                        if (proxy.getDialPrefix() == null) {
                            DialPlan dialPlan = getDialPlanForCurrentCountry();
                            if (dialPlan != null) {
                                proxy.setDialPrefix(dialPlan.getCountryCallingCode());
                            }
                        }

                        Log.e(proxy.getState());
                        if (proxy.getState() == RegistrationState.Ok) {
                            if (isGenericAccount) {
                                if (useLinphoneDefaultValues) {
                                    ...
                                    core.loadConfigFromXml(
                                            LinphonePreferences.instance()
                                                    .getDefaultDynamicConfigFile());
                                } else {
                                    // If this isn't a sip.linphone.org account, disable push
                                    // notifications and enable
                                    // service notification, otherwise incoming calls won't work
                                    // (most probably)
                                    if (proxyConfig != null) {
                                    ...
                                    LinphonePreferences.instance()
                                            .setServiceNotificationVisibility(true);
                        ...
                        } else if (proxy.getState() == RegistrationState.Failed) {
                            Toast.makeText(
                            ...
                            if (proxy.findAuthInfo() != null)
                                core.removeAuthInfo(proxyConfig.findAuthInfo());
